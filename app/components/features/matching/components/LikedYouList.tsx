...
export function LikedYouList() {
  ...
  const handleAction = async (userId: string, action: "LIKE" | "PASS", userName: string) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {likedByUsers.map((user) => (
        <div
          key={user.userId}
          className="bg-bg-card rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all"
        >
          ...
          <div className="p-4">
            ...
            {user.interests && user.interests.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {user.interests.slice(0, 3).map((interest, index) => (
                  <span
                    key={index}
                    className="text-xs bg-bg-hover text-text-body px-2 py-1 rounded-full"
                  >
                    {interest}
                  </span>
                ))}
            ...
            <div className="mt-4 pt-4 border-t border-border-main flex gap-2">
              <Button
                ...
                disabled={likeActionMutation.isPending && actingOnUser === user.userId}
              ...
              >
                ...
                {likeActionMutation.isPending && actingOnUser === user.userId ? "Liking..." : "Like Back"}
              </Button>
            </div>
          </div>
        </div>
